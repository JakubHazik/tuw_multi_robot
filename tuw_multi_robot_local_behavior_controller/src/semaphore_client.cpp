...
bool SemaphoreClient::requestAuthorization(const uint32_t & actual_segment_id, const uint32_t & restricted_segment_id) {
    ...
    if (!lights_client_in_.waitForExistence(ros::Duration(1)))
    {
        ...
        ROS_INFO_STREAM_THROTTLE(1.0, "Request authorization for segment : " << restricted_segment_id << " ( Current segment is : " << actual_segment_id << " )");
        bool srv_call_success = lights_client_in_.call(srv);
        if (srv_call_success) {
            if(srv.response.ack == true)
                ROS_INFO_STREAM_THROTTLE(1.0, ros::this_node::getName() << " : received answer from server : True");
            else if(srv.response.ack == false)            
                ROS_INFO_STREAM_THROTTLE(1.0, ros::this_node::getName() << " : received answer from server : False");
